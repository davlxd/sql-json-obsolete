  ...
  function doItems(computedCollection, toComputedCollection){
    ...
    toComputedCollection.forEach(function(itemSet){
      ...
      bnf.symbols().forEach(function(symbol){
        ...
        // console.log('      output.....');
        // console.log(gotoItemSet);
        if (gotoItemSet !== 'accept' && gotoItemSet.length > 0) {
...
function generatePropagateTable(kernels){
  var propagateTable = kernels.reduce(function(prev, itemSet){ // build 4-level empty array
...
function propagateLookahead(){
  ...
  var lookaheadTable = generatePropagateTable(kernels)[1];  lookaheadTable[0][0][0].push('EOF');
...
function lalrParsingTable(){
  ...
  acceptItem.body[0].dotIndex = 1; acceptItem.body[0].lookahead = ['EOF'];
  ...
  function handleAmbiguous(index, symbol, value){
    ...
    var reduceSymbolAmbiguousRule = bnf.ambiguousRuleFor(reduceSymbol);
    var nextSymbolAmbiguousRule = bnf.ambiguousRuleFor(symbol);

    if (reduceRule.prec)
      reduceSymbolAmbiguousRule = bnf.ambiguousRuleFor(reduceRule.prec);

    if (reduceSymbolAmbiguousRule.precedence > nextSymbolAmbiguousRule.precedence) {
      actionTable[index][symbol] = reduce;
    } else if (reduceSymbolAmbiguousRule.precedence < nextSymbolAmbiguousRule.precedence) {
      actionTable[index][symbol] = shift;
    } else if (reduceSymbolAmbiguousRule.association === nextSymbolAmbiguousRule.association){
      actionTable[index][symbol] = reduceSymbolAmbiguousRule.association === 'left'? reduce : shift;
    } else {
  ...
  for (var index = 0; index < kernels.length; index++){
    ...
    itemSet.forEach(function(item){
      if (bnf.itemEqual(item, acceptItem, 'noCheck')) actionTable[index]['EOF'] = 'accept';
...
function parsing(next){
  ...
  while(true) {
    ...
    var action = actionTable[top][token.tag];
    ...
    if (action.indexOf('shift') === 0) {
      ...
      var popIndex = symbolStack.length - reduceRule.expr.length;

      stateStack.splice(popIndex + 1, reduceRule.expr.length);
      var poppedSymbol = symbolStack.splice(popIndex, reduceRule.expr.length);
      if (reduceRule.semAction && reduceRule.semAction.apply(this, poppedSymbol)) {
        symbolStack.push({
          'tag': reduceRule.head,
          'value': reduceRule.semAction.apply(this, poppedSymbol)
          });
      } else {
        symbolStack.push({'tag': reduceRule.head});
      }
...
function slrParsingTable(){
  ...
  for (var index = 0; index < collection.length; index++){
    ...
    itemSet.forEach(function(item){
      if (bnf.itemEqual(item, acceptItem)) actionTable1[index]['EOF'] = 'accept';

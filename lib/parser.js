...
function generatePropagateTable(kernels){
  var propagateTable = {};
  kernels.forEach(function(itemSet, index){ propagateTable[index] = []; });
  ...
  function collectionWhichItemSetContains(collection, targetItem){
    for (var index = 0; index < collection.length; index++)
      for (var itemIndex = 0; itemIndex < collection[index].length; itemIndex++) {
        var item = collection[index][itemIndex];
        if (item.head !== targetItem.head)  continue;

        for (var bodyEntryIndex = 0; bodyEntryIndex < item.body.length; bodyEntryIndex++)
          if (bnf.bodyEntryEqual(item.body[bodyEntryIndex], targetItem.body[0]))  return index;
      }
    return -1;
  }
  ...
  kernels.forEach(function(itemSet, index){
    ...
    closureSet.forEach(function(item){
      item.body.forEach(function(bodyEntry){
        if (bodyEntry.dotIndex < bodyEntry.expr.length && bodyEntry.lookahead.indexOf('#') !== -1) {
          var thisItem = {};  thisItem.head = item.head;  thisItem.body = [{}];
          thisItem.body[0].expr = bodyEntry.expr;
          thisItem.body[0].dotIndex = bodyEntry.dotIndex + 1;
          var propagateIndex = collectionWhichItemSetContains(kernels, thisItem);
  ...
  return propagateTable;
}


function propagateLookahead(){
  var propagateTable = {}, lookaheadTable = [];
  var kernels = kernelItems();
  kernels.forEach(function(itemSet){
    inspectItemSet(itemSet);
  });

  var propagateTable = generatePropagateTable(kernels);
  console.log(propagateTable);
}

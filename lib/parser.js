...
function propagateLookahead(){
  ...
  var generatedTable = generatePropagateTable(kernels)[1];
  generatedTable[0].push('$');

  console.log(propagateTable);
  console.log(generatedTable);

  // for (var index = 0; index < generatedTable.length; index++){
  //   propagateTable[index].forEach(function(itemId){
  //     util.concatUniqInplace(generatedTable[itemId], generatedTable[index]);
  //   });
  // }

...
exports.augmentRule = function(lookahead){
  if (yaccRules[0].head.slice(-1) !== '_') {
    ...
    if (lookahead)
      augmentRule.body = [{'expr': [head], 'dotIndex': 0, 'lookahead': ['$']}];
    else
...
function bodyEntryEqual(bodyEntry0, bodyEntry1){
  ...
  if (bodyEntry0.lookahead && !bodyEntry1.lookahead
      || !bodyEntry0.lookahead && bodyEntry1.lookahead ) return false;

  if (bodyEntry0.lookahead && bodyEntry1.lookahead
      && bodyEntry0.lookahead.length !== bodyEntry1.lookahead.length ) return false;

  if (bodyEntry0.lookahead)
    for (var index in bodyEntry0.lookahead)
      if (bodyEntry0.lookahead[index] !== bodyEntry1.lookahead[index]) return false;
...
function generateFirstTable(){
  ...
  function first(symbol){
    ...
    getRule(symbol, yaccRules).body.forEach(function(bodyEntry){
      ...
      for (var index in bodyEntry.expr){
        concatUniqInplace(set, first(bodyEntry.expr[index]));
